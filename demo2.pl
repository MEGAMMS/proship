...
load_game(7) :-
	...
	assert(cell(1, 1, ship_left)),
	assert(cell(1, 2, ship_right)),
	assert(cell(2, 5, ship_right)),
	assert(cell(2, 4, ship_mid)).
...
check_ship_contacts :-
    \+ (
        ...
        (
            ...
            % Invalid vertical/horizontal neighbors based on type
            \+ valid_neighbors(R, C, Type)
        )
    ), !.
...
% --- Neighbor validation rules ---

valid_neighbors(R, C, submarine) :-
    % Submarine has no neighbors
    \+ has_ship_neighbor(R, C).

valid_neighbors(R, C, ship_up) :-
    R1 is R + 1,
    (cell(R1, C, ship_down);cell(R1, C, ship_mid)),
    \+ has_any_other_neighbor(R, C, [R1-C]).

valid_neighbors(R, C, ship_down) :-
    R1 is R - 1,
    (cell(R1, C, ship_up);cell(R1, C, ship_mid)),
    \+ has_any_other_neighbor(R, C, [R1-C]).

valid_neighbors(R, C, ship_left) :-
    C1 is C + 1,
    (cell(R, C1, ship_right);cell(R, C1, ship_mid)),
    \+ has_any_other_neighbor(R, C, [R-C1]).

valid_neighbors(R, C, ship_right) :-
    C1 is C - 1,
    (cell(R, C1, ship_left);cell(R, C1, ship_mid)),
    \+ has_any_other_neighbor(R, C, [R-C1]).

valid_neighbors(R, C, ship_mid) :-
    findall(R1-C1,
        (neighbor_delta(DR, DC),
         R1 is R + DR, C1 is C + DC,
         cell(R1, C1, T), ship_segment(T)),
        Neighbors),
    valid_mid_neighbors(Neighbors).

% --- Valid neighbor configurations for ship_mid
valid_mid_neighbors([R1-C1, R2-C2]) :-
    % either both same row (horizontal) or both same column (vertical)
    (R1 =:= R2 ; C1 =:= C2), !.
valid_mid_neighbors([_]).
valid_mid_neighbors(_) :-
    fail.
...
% Check if any ship neighbor exists not in Exceptions
has_any_other_neighbor(R, C, Exceptions) :-
    neighbor_delta(DR, DC),
    R1 is R + DR,
    C1 is C + DC,
    \+ member(R1-C1, Exceptions),
    cell(R1, C1, T),
    ship_segment(T), !.
...
validate_fleet :-
    % Collect all ship parts (excluding water and empty)
    forall(cell(R, C, T), (ship_segment(T), assertz(temp_ship(R, C)))),
    
    count_all_ships(0, 0, 0, 0, counts(B, C, D, S)),

    format('Found: ~w battleship(s), ~w cruiser(s), ~w destroyer(s), ~w submarine(s)~n', [B, C, D, S]),
...
validate_fleet :-
    retractall(temp_ship(_, _)),  % Cleanup
...
find_full_ship(R, C, Length) :-
    ...
    dfs(R, C, [(R,C)], ShipCells),
    length(ShipCells, Length),
    valid_ship_structure(ShipCells).  % <== validate the shape!


dfs(R, C, Visited, Final) :-
    findall((R1,C1),
        (next_ship_segment(R, C, R1, C1), \+ member((R1,C1), Visited), retract(temp_ship(R1,C1))),
        Neighbors),
    append(Visited, Neighbors, NewVisited),
    continue_dfs(Neighbors, NewVisited, Final).

continue_dfs([], Acc, Acc).
continue_dfs([(R,C)|Rest], AccIn, AccOut) :-
    dfs(R, C, AccIn, AccMid),
    continue_dfs(Rest, AccMid, AccOut).

valid_ship_structure(Cells) :-
    maplist(get_cell_type, Cells, Types),
    sort(Cells, Sorted),  % ensure ordered for pattern matching
    length(Sorted, Len),
    valid_structure_by_length(Len, Sorted, Types).

get_cell_type((R,C), Type) :- cell(R, C, Type).

valid_structure_by_length(4, [(R,C1),(R,C2),(R,C3),(R,C4)], [ship_left, ship_mid, ship_mid, ship_right]) :-
    C2 is C1 + 1, C3 is C2 + 1, C4 is C3 + 1.

valid_structure_by_length(4, [(R1,C),(R2,C),(R3,C),(R4,C)], [ship_up, ship_mid, ship_mid, ship_down]) :-
    R2 is R1 + 1, R3 is R2 + 1, R4 is R3 + 1.

valid_structure_by_length(3, [(R,C1),(R,C2),(R,C3)], [ship_left, ship_mid, ship_right]) :-
    C2 is C1 + 1, C3 is C2 + 1.

valid_structure_by_length(3, [(R1,C),(R2,C),(R3,C)], [ship_up, ship_mid, ship_down]) :-
    R2 is R1 + 1, R3 is R2 + 1.

valid_structure_by_length(2, [(R,C1), (R,C2)], [ship_left, ship_right]) :-
    C2 is C1 + 1.

valid_structure_by_length(2, [(R1,C), (R2,C)], [ship_up, ship_down]) :-
    R2 is R1 + 1.

valid_structure_by_length(1, _, [submarine]).


% Only traverse valid adjacent ship segments based on the current segment type
next_ship_segment(R, C, R1, C1) :-
    cell(R, C, Type),
    ship_segment(Type),
    valid_next_segment(Type, R, C, R1, C1),
    temp_ship(R1, C1).

% Submarine: no connections (should never get here during DFS)
valid_next_segment(_, _, _, _, _) :- fail.

valid_next_segment(ship_up, R, C, R1, C) :- R1 is R + 1, (cell(R1, C, ship_down);cell(R1, C, ship_mid)).
valid_next_segment(ship_down, R, C, R1, C) :- R1 is R - 1, (cell(R1, C, ship_up);cell(R1, C, ship_mid)).

valid_next_segment(ship_left, R, C, R, C1) :- C1 is C + 1, (cell(R, C1, ship_right);cell(R, C1, ship_mid)).
valid_next_segment(ship_right, R, C, R, C1) :- C1 is C - 1, (cell(R, C1, ship_left);cell(R, C1, ship_mid)).

valid_next_segment(ship_mid, R, C, R1, C) :-
    (R1 is R + 1 ; R1 is R - 1),
    (cell(R1, C, ship_mid);cell(R1, C, ship_up);cell(R1, C, ship_down)).
valid_next_segment(ship_mid, R, C, R, C1) :-
    (C1 is C + 1 ; C1 is C - 1),
    (cell(R, C1, ship_mid);cell(R, C1, ship_right);cell(R, C1, ship_left)).
